...
# Streamlit app
def main():
    st.title("Promptulate Model Compare Playground")
    st.subheader("Powered by [Promptulate](https://github.com/Undertone0809/Promptulate)")
    ...
    model_names = ["gpt-3.5-turbo", "gpt-4", "ernie-bot-turbo"]  # Add your model names here
    ...
    for idx, model_name in enumerate(model_names):
        with cols[idx]:
            st.text_area(label=f"{model_name}", value=outputs[idx], height=300,
                         key=f"output_{model_name}_{idx}")  # Use a unique key
