...
def main():
    ...
    st.subheader(
        "Powered by [Promptulate](https://github.com/Undertone0809/Promptulate)"
    )
    ...
    if submit_button and prompt:
        for idx, model_name in enumerate(model_names):
            thread = threading.Thread(
                target=get_model_output_thread, args=(prompt, model_name, outputs, idx)
            )
    ...
    for idx, model_name in enumerate(model_names):
        with cols[idx]:
            st.text_area(
                ...
                value=outputs[idx],
                height=300,
                key=f"output_{model_name}_{idx}",
            )  # Use a unique key
